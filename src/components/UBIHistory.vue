...
export default defineComponent({
  ...
  setup () {
    ...
    async function init (params) {
      ...
      const paymentsRes = await system.$commonFun.sendRequest(`${store.state.networkValue === 'Proxima' ? process.env.VUE_APP_UBI : process.env.VUE_APP_UBI_MAINNET}v1/rewards?${system.$Qs.stringify(paramsCont)}`, 'get') //?public_address=${store.state.metaAddress}
